...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Fetch Wikidata labels via QLever in multiple languages for disjoint "
    ...
    parser.add_argument("--batch-size",  type=int, default=100, help="Batch size for QLever queries (default: 100).")
...
def build_statements(
    ...
    if nature_type == "negative":
        if version == "v1":
            context_relation = disjoint_with_label
            statement_relation = f"NOT {relation_label}"
        else:
            context_relation = f"NOT {disjoint_with_label}"
            statement_relation = relation_label
    else:
        context_relation = disjoint_with_label
        statement_relation = relation_label
    ...
    for _, row in df.iterrows():
        label_subj  = get_label(labels, str(row[subj_col]))
        ...
        rows.append({
            ...
            "relation2": f"({label_subj}; {statement_relation}; {label_disj2})",
...
def main():
    ...
    subj_col    = "violating_subclass" if args.disjointeness_type == "class" else "violating_instance"
    DATA_COLUMNS[2] = subj_col
    ...
    for friendly_name, lang_code in lang_pairs:
        ...
        version = "v1" if "v1" in args.input else "v2"
        statements = build_statements(final_df, labels, relation_label, disjoint_with_label, args.nature_type, version, subj_col)

        nature = "contradiction" if args.nature_type == "positive" else "non_contradiction"
        if args.disjointeness_type == "class":
            out_path = os.path.join(args.output_dir, f"disjoint_{args.disjointeness_type}_{nature}_{friendly_name}.tsv")
        else:
            out_path = os.path.join(args.output_dir, f"disjoint_{args.disjointeness_type}_{nature}_{version}_{friendly_name}.tsv")
